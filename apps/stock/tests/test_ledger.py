...
class LedgerTests(TestCase):
    def setUp(self):
        self.lb = Unit.objects.create(code="lb", name="Pound", kind=UnitKind.WEIGHT, to_canonical=Decimal("453.59237"))
        self.gal = Unit.objects.create(code="gal", name="Gallon", kind=UnitKind.VOLUME, to_canonical=Decimal("3785.41"))
        self.storage = Location.objects.create(code="storage", name="Devonshire St", kind=Location.Kind.STORAGE)
    ...
    def test_balance_is_the_sum_of_movements(self):
        post_movement(item=self.dal, location=self.storage, quantity=Decimal("150"), movement_type=MovementType.RECEIPT)
        post_movement(item=self.dal, location=self.storage, quantity=Decimal("-18"), movement_type=MovementType.TRANSFER_OUT)
        self.assertEqual(on_hand(self.dal, self.storage), Decimal("132"))

    def test_cache_can_always_be_rebuilt_from_the_ledger(self):
        post_movement(item=self.dal, location=self.storage, quantity=Decimal("100"), movement_type=MovementType.RECEIPT)
        from apps.stock.models import StockBalance

        StockBalance.objects.update(quantity=Decimal("999999"))  # corrupt it
        rebuild_balances()
        self.assertEqual(StockBalance.objects.get(item=self.dal, location=self.storage).quantity, Decimal("100"))

    def test_a_correction_is_a_new_row_not_an_edit(self):
        m = post_movement(item=self.dal, location=self.storage, quantity=Decimal("50"), movement_type=MovementType.RECEIPT)
    ...
    def test_transfer_moves_stock_between_locations_atomically(self):
        post_movement(item=self.dal, location=self.storage, quantity=Decimal("150"), movement_type=MovementType.RECEIPT)
        t = Transfer.objects.create(from_location=self.storage, to_location=self.rest, occurred_at="2026-09-16T15:00:00Z")
    ...
    def test_negative_stock_can_be_refused(self):
        with self.assertRaises(StockError):
            post_movement(
                item=self.dal, location=self.rest, quantity=Decimal("-5"),
                movement_type=MovementType.WASTE, allow_negative=False,
            )
...
class ExplosionTests(TestCase):
    ...
    def setUp(self):
        self.lb = Unit.objects.create(code="lb", name="Pound", kind=UnitKind.WEIGHT, to_canonical=Decimal("453.59237"))
        self.gal = Unit.objects.create(code="gal", name="Gallon", kind=UnitKind.VOLUME, to_canonical=Decimal("3785.41"))
        self.each = Unit.objects.create(code="each", name="Each", kind=UnitKind.COUNT, to_canonical=Decimal("1"))

        self.rice = Item.objects.create(code="raw-rice", name="Idli rice", kind=ItemKind.RAW, base_unit=self.lb)
        self.dal = Item.objects.create(code="raw-urad", name="Urad dal", kind=ItemKind.RAW, base_unit=self.lb)
        self.batter = Item.objects.create(code="prep-dosa-batter", name="Dosa batter", kind=ItemKind.PREPARED, base_unit=self.gal)
        self.dosa = Item.objects.create(code="dish-plain-dosa", name="Plain dosa", kind=ItemKind.DISH, base_unit=self.each, is_stocked=False)
